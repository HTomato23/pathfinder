<x-layout.app title="Consulation">
    <x-layout.admin.admin-sidebar />
    <main class="flex flex-col gap-6 p-5 xl:ml-[256px]">
        <x-layout.admin.admin-navbar page="Consultation" />
                ...
                <div x-cloak :class="$store.theme.isDark() ? 'bg-base-200' : 'bg-base-100'"  class="rounded-sm shadow-sm p-4">
                    <div class="flex flex-col gap-2">
                       ...
                       <div class="flex flex-col gap-1 mt-2 text-sm">
                        ...
                        <div class="flex justify-between text-xs mt-3 mb-2 opacity-70">
                            <span>{{ $item->admin->first_name }} {{ $item->admin->last_name }}</span>
                            <span>{{ $item->updated_at->format('F j, Y \a\t g:i A') }}</span>
                        </div>

                        <hr />

                        <div class="flex justify-end mt-2">
                            <x-ui.button href="/admin/dashboard/consultation/{{ $item->id }}" x-cloak x-bind:class="$store.theme.isDark() ? 'btn-soft' : ''" color="primary" size="sm">Edit</x-ui.button>
                        </div>
                    </div>
                </div>
            @endforeach
