<x-layout.client title="Academic">
    ...
    <main class="flex flex-col gap-6 p-5 xl:ml-[256px]">
        ...
        <!-- Questions Form -->
        <form id="academic-test-form" method="POST" action="{{ route('dashboard.assessment.academic.update') }}">
            @csrf
            @method('PATCH')
            <x-layout.client.client-academic />
        </form>

        <!-- Cancel Form -->
        <form id="cancel-form" class="hidden" method="POST" action="{{ route('dashboard.assessment.academic.cancel') }}">
            @csrf
        </form>
