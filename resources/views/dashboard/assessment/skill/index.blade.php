<x-layout.client title="Skill Scale Test">
    <x-layout.client.client-sidebar></x-layout.client.client-sidebar>
    <main class="flex flex-col gap-6 p-5 xl:ml-[256px]">
        <x-layout.client.client-navbar page="Skill Scale Test"></x-layout.client.client-navbar>
        ...
        @if (Auth::user()->program === 'BSHM')
            <!-- Questions Form -->
            <form id="skill-test-form" method="POST" action="{{ route('dashboard.assessment.skill.update') }}">
                @csrf
                @method('PATCH')
                <x-layout.client.client-skill-scale :skill_scale_field="$skill_scale_field_hm" :skill_scale="$skill_scale" />
            </form>
        @else
            <!-- Questions Form -->
            <form id="skill-test-form" method="POST" action="{{ route('dashboard.assessment.skill.update') }}">
                @csrf
                @method('PATCH')
                <x-layout.client.client-skill-scale :skill_scale_field="$skill_scale_field" :skill_scale="$skill_scale" />
            </form>
        @endif
        ...
        <!-- Cancel Form -->
        <form id="cancel-form" class="hidden" method="POST" action="{{ route('dashboard.assessment.skill.cancel') }}">
            @csrf
        </form>
